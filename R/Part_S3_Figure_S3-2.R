...
scam_plots <- ggplot(data = my.models.predictions, aes(x = Biostim, y = Fit, color = Stratum)) +
  geom_ribbon(aes(ymin = Fit - 1.96 * SE, ymax = Fit + 1.96 * SE, group = Stratum), color = NA, alpha = 0.2, fill = "#39568cff", show.legend = FALSE) +
...
ggsave("figures/Part_S3_Figure_S3-2.jpg", plot = scam_plots, height = 6, width = 10)
